...
function StaffManagement({ user }) {
  ...
  const fetchBranches = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/branches', { headers });
      if (response.ok) {
        const data = await response.json();
...
function StaffForm({ staff, branches, onSave, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = staff
        ? `http://localhost:5000/api/staff/${staff.staff_id}`
...
function Loans({ user }) {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/loans', { headers })
      .then(res => res.json())
      .then(data => {
        setLoans(data.data || []);
        setLoading(false);
...
function Fines({ user }) {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/fines', { headers })
      .then(res => res.json())
      .then(data => {
        setFines(data || []);
        setLoading(false);
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        const response = await fetch(`http://localhost:5000/api/fines/${fine.fine_id}`, {
                          method: 'PATCH',
                          headers,
                        ...
                        if (response.ok) {
                          // Refresh fines list
                          const finesResponse = await fetch('http://localhost:5000/api/fines', { headers });
